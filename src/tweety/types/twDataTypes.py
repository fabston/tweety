    ...
    async def translate(self):
        if not self._translated_text:
            ...
            self._translated_text = (
                response.get("data", {}).get("birdwatch_note_by_rest_id", {}).get("translated_summary", {}).get("text")
            )
...
class Tweet(_TwType):
    ...
    def get_threads(self):
        ...
        for entry in entries:
            if str(entry["entryId"].split("-")[0]) == "conversationthread":
                ...
                str(entry["entryId"].split("-")[0]) == "tweet"
                and entry["content"]["itemContent"]["tweetDisplayType"] == "SelfThread"
            ):
                try:
    ...
    async def get_comments(
        self, pages=1, wait_time=2, cursor=None, get_hidden=False, filter_=TweetCommentFilters.Relevant
    ):
    ...
    def _check_if_protected(self):
        ...
        if is_protected and is_protected.get("reason") == "Protected":
            ...
            raise AuthenticationRequired(
                ...
                "NsfwLoggedOut",
                response=self._raw,
                message="This Tweet is flagged as NSFW, make sure you are logged-in and birthday is updated in your account.",
            )
        elif is_protected and is_protected.get("reason") == "Suspended":
            raise UserProtected(
                error_code="UserSuspended", response=self._raw, message="The Author of this Tweet is Suspended"
            )
    ...
    def _get_community(self):
        return Community(
            self._client, self._tweet.get("author_community_relationship") or self._tweet.get("community_results")
        )
    ...
    def _get_tweet_media(self):
        ...
        media = []
        media.extend(
            Media(self._client, media) for media in self._original_tweet.get("extended_entities", {}).get("media", [])
        )
    ...
    def _get_tweet_mentions(self):
        users = [
            ShortUser(self._client, user) for user in self._original_tweet.get("entities", {}).get("user_mentions", [])
        ]
...
class RichText(_TwType):
    ...
    async def get_html(self):
        ...
        ordered_tags = sorted(
            tags, key=lambda x: x.from_index if hasattr(x, "from_index") else x["index"], reverse=True
        )
        thisHtml = self.text
...
class Media(_TwType):
    def __init__(self, client, media_dict, *args, **kwargs):
        ...
        self.sizes = [
            MediaSize(self._client, v, k) for k, v in self._raw.get("sizes", {}).items() if self._raw.get("sizes")
        ]
        ...
        self.tagged_users = self.tags = [
            ShortUser(self._client, i) for i in find_objects(self.features, "tags", None, none_value=[])
        ]
        self.streams = []
...
class User(_TwType):
    def __init__(self, client, user_data, *args, **kwargs):
        ...
        self.created_at = self.date = parse_time(self._original_user.get("created_at")) or parse_time(
            self._core.get("created_at")
        )
    ...
    async def search_tweets(
        ...
        return await self._client.search(
            f"from:{self.username} {keyword}", pages=pages, filter_=filter_, wait_time=wait_time, cursor=cursor
        )
...
class Topic(_TwType):
    def __init__(self, client, topic):
        self._client = client
        self._raw = topic
        self.original_topic = find_objects(
            self._raw, "__typename", ["TopicPageHeader", "TimelineTopic"], recursive=False
        )
...
class GrokShare(_TwType):
    def __init__(self, client, grok_share_json):
        ...
        self.messages = [
            GrokShareMessage(self._client, i) for i in self._grok_share_data.get("conversation_preview", [])
        ]
